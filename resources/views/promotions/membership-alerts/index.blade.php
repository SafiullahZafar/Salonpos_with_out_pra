...
.cust-av{width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,#7C3AED,#6D28D9);color:#fff;font-size:.72rem;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
...
.action-send{color:#7C3AED;font-size:.8rem;font-weight:600;background:none;border:none;cursor:pointer;padding:5px 9px;border-radius:7px;font-family:"Inter",sans-serif;transition:.15s;}
.action-send:hover{background:#F5F3FF;}
...
<div class="table-wrap">
  ...
  <table class="promo-table">
    ...
    <tr>
      ...
      <td><div style="font-size:.82rem;">{{ \Carbon\Carbon::parse($alert->alert_date)->format("M d, Y") }}</div>@if(\Carbon\Carbon::parse($alert->alert_date)->isPast() && !$alert->is_sent)<div style="font-size:.7rem;color:#6D28D9;font-weight:600;">Overdue</div>@endif</td>
      <td><div style="font-size:.82rem;">{{ \Carbon\Carbon::parse($alert->membership_expiry_date)->format("M d, Y") }}</div>@if(\Carbon\Carbon::parse($alert->membership_expiry_date)->isPast())<div style="font-size:.7rem;color:#6D28D9;font-weight:600;">Expired</div>@endif</td>
