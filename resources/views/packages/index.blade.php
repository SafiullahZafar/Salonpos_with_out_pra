...
.btn-solid{padding:9px 18px;border:none;background:linear-gradient(135deg,#6D28D9,#6D28D9);border-radius:10px;color:#fff;font-size:.85rem;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;transition:.2s;box-shadow:0 3px 10px rgba(109,40,217,.25);text-decoration:none;display:inline-flex;align-items:center;gap:6px;}
.btn-solid:hover{transform:translateY(-1px);box-shadow:0 5px 16px rgba(109,40,217,.35);}
.btn-outline{padding:9px 18px;border:1.5px solid #e2e8f0;background:#fff;border-radius:10px;color:#64748b;font-size:.85rem;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:6px;transition:.2s;font-family:'Inter',sans-serif;}
.btn-outline:hover{border-color:#6D28D9;color:#6D28D9;background:#F5F3FF;}

/* Package cards */
.pkg-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:16px;}

.pkg-card{background:#fff;border:1px solid #DDD6FE;border-radius:16px;box-shadow:0 1px 4px rgba(0,0,0,.04);overflow:hidden;transition:box-shadow .2s;display:flex;flex-direction:column;}
...
.pkg-icon{width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#F5F3FF,#EDE9FE);display:flex;align-items:center;justify-content:center;color:#6D28D9;flex-shrink:0;}
...
.pkg-price{font-size:1.3rem;font-weight:800;color:#6D28D9;line-height:1;}
.pkg-price-label{font-size:.68rem;color:#94a3b8;margin-top:2px;}
.pkg-peak-price{font-size:.78rem;font-weight:600;color:#8B5CF6;margin-top:3px;}
...
.chip-active{background:#EDE9FE;color:#4C1D95;}
.chip-inactive{background:#f1f5f9;color:#64748b;}
.chip-duration{background:#F5F3FF;color:#6D28D9;}
.chip-count{background:#F5F3FF;color:#4C1D95;}
...
.service-dot{width:6px;height:6px;border-radius:50%;background:#6D28D9;flex-shrink:0;}
...
.action-del{color:#6D28D9;font-size:.8rem;font-weight:600;background:none;border:none;cursor:pointer;padding:5px 10px;border-radius:7px;font-family:'Inter',sans-serif;transition:.15s;display:inline-flex;align-items:center;gap:4px;}
.action-del:hover{background:#F5F3FF;}

.empty-state{background:#fff;border:1px solid #DDD6FE;border-radius:16px;padding:60px 20px;text-align:center;color:#cbd5e1;}
...
.modal-box{background:#fff;border-radius:18px;width:100%;max-width:400px;padding:26px;box-shadow:0 20px 50px rgba(0,0,0,.15);border:1px solid #EDE9FE;}
.modal-title{font-size:1.15rem;font-weight:800;color:#6D28D9;margin-bottom:10px;}
...
.btn-m-delete:hover{background:#4C1D95;transform:translateY(-1px);}
</style>

<x-ui.page-header title="Package Deals" subtitle="Bundle services together with custom pricing">
    <x-slot:actions>
...
<div class="pkg-card">
    ...
    <div class="pkg-card-body">
        <div class="pkg-chips">
            ...
            <span class="pkg-chip" style="background:#F5F3FF;color:#6D28D9;">Peak price active</span>
            @endif
        </div>
        ...
        <div class="pricing-grid">
            <div class="pricing-item">
                ...
                <div class="pricing-item-val {{ isset($package->pricing_levels['junior']) ? '' : 'muted' }}">
                    {{ isset($package->pricing_levels['junior']) ? 'PKR '.number_format($package->pricing_levels['junior'],2) : 'â€”' }}
                </div>
            </div>
            <div class="pricing-item">
                <div class="pricing-item-label">Senior</div>
                <div class="pricing-item-val {{ isset($package->pricing_levels['senior']) ? '' : 'muted' }}">
                    {{ isset($package->pricing_levels['senior']) ? 'PKR '.number_format($package->pricing_levels['senior'],2) : 'â€”' }}
                </div>
            </div>
            <div class="pricing-item">
                <div class="pricing-item-label">Master</div>
                <div class="pricing-item-val {{ isset($package->pricing_levels['master']) ? '' : 'muted' }}">
                    {{ isset($package->pricing_levels['master']) ? 'PKR '.number_format($package->pricing_levels['master'],2) : 'â€”' }}
                </div>
            </div>
        </div>
        @endif
        ...
        <div style="font-size:.75rem;color:#94a3b8;display:flex;align-items:center;gap:5px;margin-bottom:14px;">
            ...
            Peak hours: {{ $package->peak_start }} â€“ {{ $package->peak_end }}
        </div>
        @endif
...
<div class="empty-state">
    ...
    <p>No packages yet â€” create your first bundle</p>
</div>
@endforelse
