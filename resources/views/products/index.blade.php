...
.fg select,.fg input{padding:8px 10px;border:1.5px solid #DDD6FE;border-radius:9px;font-size:.84rem;font-family:'Inter',sans-serif;background:var(--ybg);color:#18181b;outline:none;transition:.2s;}
.fg select:focus,.fg input:focus{border-color:var(--y1);background:#fff;box-shadow:0 0 0 3px rgba(109,40,217,.15);}
.search-row{display:flex;gap:7px;}
.btn-search{padding:8px 14px;background:#18181b;color:#fff;border:none;border-radius:9px;font-weight:700;font-size:.82rem;cursor:pointer;display:flex;align-items:center;gap:5px;font-family:'Inter',sans-serif;transition:.2s;white-space:nowrap;}
...
.chip-low{background:#EDE9FE;border-color:#DDD6FE;color:#4C1D95;}
.chip-out{background:#EDE9FE;border-color:#A78BFA;color:#3B0764;}

/* â”€â”€ Table wrapper â”€â”€ */
.tbl-wrap{background:#fff;border:1.5px solid var(--y1);border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(109,40,217,.12);}
.tbl-scroll{overflow-x:auto;}

/* â”€â”€ Excel-style table â”€â”€ */
...
/* Row styles */
.products-tbl tbody tr{border-bottom:1px solid #F5F3FF;transition:background .13s;}
.products-tbl tbody tr:nth-child(even){background:#F8F7FA;}
.products-tbl tbody tr:hover{background:#F5F3FF;}
.products-tbl tbody tr:last-child{border-bottom:none;}
.products-tbl td{padding:10px 14px;font-size:.84rem;color:#27272a;vertical-align:middle;border-right:1px solid #DDD6FE;}
...
.s-low{background:#EDE9FE;color:#4C1D95;}
.s-out{background:#EDE9FE;color:#3B0764;}

/* Actions column */
.actions-cell{display:flex;align-items:center;gap:5px;flex-wrap:nowrap;}
.btn-act{padding:5px 10px;border-radius:7px;text-align:center;text-decoration:none;font-size:.72rem;font-weight:700;transition:.15s;border:1.5px solid transparent;cursor:pointer;white-space:nowrap;font-family:'Inter',sans-serif;display:inline-flex;align-items:center;gap:4px;}
...
.btn-stock{background:linear-gradient(135deg,#18181b,#3f3f46);color:#6D28D9;border-color:#18181b;}
...
.btn-del{background:transparent;color:#4C1D95;border-color:#A78BFA;}
...
.modal-form-group input, .modal-form-group textarea {width:100%;padding:10px 14px;border:1.5px solid #e4e4e7;border-radius:9px;font-size:.9rem;font-family:'Inter',sans-serif;box-sizing:border-box;}
.modal-form-group input:focus, .modal-form-group textarea:focus {border-color:var(--y1);outline:none;box-shadow:0 0 0 3px rgba(109,40,217,.15);}
.modal-footer{display:flex;gap:10px;margin-top:20px;}
.btn-m{flex:1;padding:12px;border-radius:9px;font-size:.9rem;font-weight:700;cursor:pointer;transition:.2s;border:none;font-family:'Inter',sans-serif;}
...
/* Pagination */
.pag-wrap{padding:12px 16px;border-top:1px solid #DDD6FE;}
...
{{-- Header --}}
<x-ui.page-header title="Products" description="Manage catalogue details, prices and current stock levels." eyebrow="Catalog & stock">
    <x-slot:actions>
...
<form method="GET" class="filters-bar">
    ...
    <div class="fg">
        ...
        <div class="search-row">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Name, SKU or barcodeâ€¦" style="flex:1;">
        ...
        <table class="products-tbl">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    {{-- Cost Price --}}
                    <td class="td-center cost-cell">{{ $product->cost_price ? 'PKR '.number_format($product->cost_price,0) : 'â€”' }}</td>
                ...
                <tr>
                    <td colspan="9">
                        <div class="empty-state">
                            ...
                            <p>No products found â€” try adjusting your filters</p>
                        </div>
                    </td>
                </tr>
                @endforelse
