...
.svc-card{background:#fff;border:1px solid #DDD6FE;border-radius:16px;box-shadow:0 1px 4px rgba(0,0,0,.04);overflow:hidden;transition:box-shadow .2s;}
...
.svc-icon{width:42px;height:42px;border-radius:12px;background:#F5F3FF;display:flex;align-items:center;justify-content:center;color:#6D28D9;flex-shrink:0;}
...
.svc-price{font-size:1.2rem;font-weight:800;color:#6D28D9;line-height:1;}
.svc-price-label{font-size:.68rem;color:#94a3b8;margin-top:2px;}
.svc-peak-price{font-size:.78rem;font-weight:600;color:#8B5CF6;margin-top:3px;}
...
.chip-popular{background:#EDE9FE;color:#4C1D95;}
.chip-peak{background:#F5F3FF;color:#6D28D9;}
...
.action-del{color:#6D28D9;font-size:.8rem;font-weight:600;background:none;border:none;cursor:pointer;padding:5px 10px;border-radius:7px;font-family:'Inter',sans-serif;transition:.15s;}
...
<x-ui.page-header title="Services" description="Manage services, pricing tiers and peak-hour rates." eyebrow="Catalog">
    <x-slot:actions>
    ...
    <div class="svc-card">
        ...
        <div class="svc-card-body">
            <div class="svc-meta-row">
                ...
                <span class="svc-chip chip-popular">â­ Popular</span>
                @endif
            ...
            <div class="pricing-grid">
                <div class="pricing-item">
                    ...
                    <div class="pricing-item-val {{ isset($service->pricing_levels['junior']) ? '' : 'muted' }}">
                        {{ isset($service->pricing_levels['junior']) ? 'PKR '.number_format($service->pricing_levels['junior'],2) : 'â€”' }}
                    </div>
                </div>
                <div class="pricing-item">
                    <div class="pricing-item-label">Senior</div>
                    <div class="pricing-item-val {{ isset($service->pricing_levels['senior']) ? '' : 'muted' }}">
                        {{ isset($service->pricing_levels['senior']) ? 'PKR '.number_format($service->pricing_levels['senior'],2) : 'â€”' }}
                    </div>
                </div>
                <div class="pricing-item">
                    <div class="pricing-item-label">Master</div>
                    <div class="pricing-item-val {{ isset($service->pricing_levels['master']) ? '' : 'muted' }}">
                        {{ isset($service->pricing_levels['master']) ? 'PKR '.number_format($service->pricing_levels['master'],2) : 'â€”' }}
                    </div>
                </div>
            </div>
            @endif
            ...
            <div style="font-size:.75rem;color:#94a3b8;display:flex;align-items:center;gap:5px;">
                <svg width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                Peak hours: {{ optional($service->peak_start)->format('H:i') }} â€“ {{ optional($service->peak_end)->format('H:i') }}
            </div>
            @endif
        </div>
...
<div class="empty-state">
    ...
    <p>No services yet â€” create your first one</p>
</div>
@endif
